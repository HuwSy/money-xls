    ...
    async function setupSpentFields() {
      ...
      if (last[0][0] == "")
        last[0] = [begining.getFullYear(), begining.getMonth() + 1, begining.getDate()];
    ...
    async function Saving() {
      ...
      // saving file
      var format = window.name.substring(window.name.lastIndexOf('.') + 1);
      var wb = XLSX.write(workbook, { bookType: format, cellStyles: true, compression: true, bookSST: true, dense: true, type: 'binary' });
      var buf = s2ab(wb);
      var blob = new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", charset: "UTF-8", name: window.name });

      // downloading
      var aElement = document.createElement('a');
      aElement.innerHTML = window.name;
      aElement.download = window.name;
      aElement.setAttribute('download', window.name);
    ...
    // Mimic google sheets api that this was originally developed against
